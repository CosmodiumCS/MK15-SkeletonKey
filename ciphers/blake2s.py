...
+-------------------------------------------------------------+-----------+
| [✓] Example:                                                            |
|  cryptex blake2s -e -t hello                                            |
|  cryptex blake2s -b -t 5d41402abc4b2a76b9719d911017c592 -w example.tx   |
|  ceyptex blake2s -b -t 5d41402abc4b2a76b9719d911017c592 -r 13           |
+-------------------------------------------------------------------------+
"""
...
def encode(args):
    ...
    if text:
        ...
        # Detect Salt
        if salt:
            result = hashlib.blake2s( salt.encode('ascii') +
                                 text.encode('ascii')  ).hexdigest()
            output += f'Salt | {salt}'
        else:
            result = hashlib.blake2s( text.encode('ascii')  ).hexdigest()
