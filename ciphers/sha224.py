...
|  cryptex sha224 -b -t 5d41402abc4b2a76b9719d911017c592 -w example.txt   |
|  ceyptex sha224 -b -t 5d41402abc4b2a76b9719d911017c592 -r 13            |
+-------------------------------------------------------------------------+
"""
...
def encode(args):
    ...
    if text:
        ...
        # Detect Salt
        if salt:
            result = hashlib.sha224( salt.encode('ascii') +
                                 text.encode('ascii')  ).hexdigest()
            output += f'Salt | {salt}'
        else:
            result = hashlib.sha224( text.encode('ascii')  ).hexdigest()
